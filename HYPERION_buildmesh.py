...
st_time = float(input('What is the start time of the simulation (seconds)?'))
end_time = float(input('What is the end time of the simulation (seconds)?'))
t_steps = int(input('How many time steps are in the simulation?'))
times = np.linspace(st_time, end_time, num=t_steps)  # stores time values
t = times[1] - times[0]  # delta_t or dt
time_frames = len(times)  # Total Number of time frames

st_rad = float(input('What is the starting radius of the slab (meters)?'))
end_rad = float(input('What is the ending radius of the slab (meters)?'))
r_nodes = int(
    input('What is the number of nodes in your r (radial) dimension?'))
radius_m = np.linspace(st_rad, end_rad, num=r_nodes)
r = radius_m[1] - radius_m[0]  # Mesh width units in r, or dr
...
alpha = 6e4  # heat transmission coeff (MW/m^2-K)
w = 10  # Surface layer correction. Makes poorly adhered surface layer thin.
...
# The following are thermal constants and properties
...
# Only extract the amount of data needed. r_nodes is the 1D radial span
...
with open(temp_file) as csv_file:
    ...
    i = 0
    for row in csv_temp:
        span = len(row)  # the amount of nodes in radial span; must match mesh
        if i < time_frames:
            for j in range(span):
...
# or shape(y,r,time) or shape(y_nodes,r_nodes,time) Note that this is in 3D.
...
# Temperatures from previous time steps. Recall Ax = b, this is b.
temp_left = np.zeros(y_nodes*r_nodes)

# The future temperature that need to be calculated. Recall Ax = b, this is x.
temp_right = np.zeros(y_nodes*r_nodes)

# The temperatures on all nodes. This is just to store ALL the temperatures.
# It isn't used in any calcs
...
# not r bc time interval is small.
    ...
    for i in range(r_nodes):
        ...
        temp_left[(y_nodes-1)*r_nodes + i] = temp_1d[1, 0]
        templeft_all[(y_nodes-1)*r_nodes + i, j] = temp_1d[1, 0]
...
# integrated heat flux decay length.
def integrate(y_vals, dx):  # y_vals is an array and h is step_size
...
with open(flux_expan) as csv_file:
    ...
    for row in csv_reader:
        ...
        i += 1
# creates an interpolated function for finding flux expansion. fx_interp == fcn
...
t_dim = 100  # Time step that you want to observe in plots
...
r_dim = peak_index + 1  # the radial node of interest for heat flux evolution
q_peak = q_peaks[t_dim]
r_peak = r_peaks[t_dim]  # Get the radius of the peak
